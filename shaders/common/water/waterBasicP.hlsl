...
// specularParams
#define SPEC_ROUGHNESS     specularParams[3]
...
// Uniforms
//-----------------------------------------------------------------------------
uniform_sampler2D( bumpMap, 0 );
//uniform_sampler2D( prepassTex , 1 );
uniform_sampler2D( reflectMap , 2 );
uniform_sampler2D( refractBuff, 3 );
uniform_samplerCUBE( skyMap, 4 );
//uniform_sampler2D( foamMap, 5 );
...
float4 main( ConnectData IN ) : SV_TARGET
{
   ...
   // Get sun luminance
   float skyMapFactor = saturate(hdrLuminance(sunColor.rgb));

   // If we do not have a reflection texture then we use the cubemap.
   float4 cubeMapColor = lerp( waterBaseColor, texCUBE( skyMap, reflectionVec ) * skyMapFactor * float4(SPEC_COLOR, 1), hasCubemap );
   refMapColor = lerp( refMapColor, cubeMapColor, NO_REFLECT );
   ...
   float4 OUT = lerp( diffuseColor, reflectColor, fresnelTerm );
      ...
      specular = pow( specular, glossToPower(1 - SPEC_ROUGHNESS) );
   ...
   float3 subsurfaceColor = vec3(0.0, 0.0, 0.0);

   // Calculate the subsurface scattering term
   if (dot(LIGHT_VEC, vec3(0, 0, 1)) > 0.0) {
      float scatterAmount = (1.0 - pow(max(0.0, dot(normalize(LIGHT_VEC), vec3(0, 0, 1))), 5.0)) * 0.8;

      subsurfaceColor = vec3(0.5, 1.0, 1.0) * scatterAmount;
   }

   // Combine the subsurface scattering term with the original color
   OUT += subsurfaceColor;
