...
#endif
   float horizonWeight = pow(max(0, dot(lightDir, float3(0.0,0.0,1.0))), 2);
   float3 ld = lightDir * horizonWeight + float3(0.0,0.0,1.0-horizonWeight);

   float fCos = dot( ld, In.v3Direction ) / length(In.v3Direction);
