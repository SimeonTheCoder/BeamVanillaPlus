...
float vernierScale(float fCos)
{
    ...
    return 0.35 * outx + 0.65 * (0.25 * outx);
}
...
Conn main( Vert In )
{
   ...
   float4 v3Pos = float4(In.position,1) / 6378000.0; // outerRadius;
   ...
   Out.position = mul( modelView, float4(In.position,1) );
   Out.mieColor.rgb = max(0, min(1, v3FrontColor * mieBrightness - float3(0, 0.05, 0)));
   Out.mieColor.a = 1.0f;
   Out.rayleighColor.rgb = max(0, min(1, v3FrontColor * (invWaveLength.xyz * rayleighBrightness) - float3(0, 0.05, 0)));
