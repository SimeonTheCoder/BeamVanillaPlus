#ifndef _SOFT_SHADOWMAP_H_HLSL_
#define _SOFT_SHADOWMAP_H_HLSL_
...
#if !defined(BNG_USE_SCENE_CBUFFER)
uniform_sampler2D( gTapRotationTex, 4);
SamplerTexture2D getTapRotationTex() {
   return gTapRotationTex;
}
#else
SamplerTexture2D getTapRotationTex() {
   return createSamplerTexture2D(defaultSampler2D, gTapRotationTex);
}
#endif
                              ...
                              int endTap )
{
   ...
   for ( int t = startTap; t < endTap; t++ )
   {
      ...
      float depthDistance = distToLight;

      tap.x = ( sNonUniformTaps[t].x * sinCos.y - sNonUniformTaps[t].y * sinCos.x ) * filterRadius * depthDistance * 3;
      tap.y = ( sNonUniformTaps[t].y * sinCos.y + sNonUniformTaps[t].x * sinCos.x ) * filterRadius * depthDistance * 3;
      occluder = tex2Dlod( shadowMap, float4( shadowPos + tap, 0, 0 ) ).r;

      #ifdef BNG_USE_REVERSED_DEPTH_BUFFER
         occluder = 1.0f - occluder;
      #endif
                           ...
                           float esmFactor )
{
      ...
      float2 sinCos = ( tex2Dlod( getTapRotationTex(), float4(vpos * 16,0,0) ).rg - 0.5 ) * 2;
